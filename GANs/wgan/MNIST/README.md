# Introduction
These sample .nml files are for training a wasserstein gan model using image data in [NeoPulse™ AI Studio](https://aws.amazon.com/marketplace/pp/B074NDG36S/ref=vdr_rf).
...
```bash
$ neopulse train -p <project_name> -f /DM-Dash/NeoPulse_Examples/GANs/wgan/MNIST/mnist_wgan.nml
```
...
```bash
bind = "/DM-Dash/NeoPulse_Examples/GANs/wgan/MNIST/training_data.csv" ;
```
...
**mnist_wgan.nml:** Full self-defined architecture without any automation.

**mnist_wgan_auto.nml:** Features full use of the auto keyword to automatically generate the entire architecture.
